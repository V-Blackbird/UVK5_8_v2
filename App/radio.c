...
void RADIO_ConfigureSquelchAndOutputPower(VFO_Info_t *pInfo)
{
    ...
    {
        if(gSetting_set_pwr == OUTPUT_POWER_MID)
            Op = 1; // 5W
        else if(gSetting_set_pwr == OUTPUT_POWER_HIGH)
            Op = 2; // 8W
        else
            Op = 0; // all other (Low) powers, including LOW_6 (2W)
    ...
    */
    ...
    static const uint8_t dividers[7] = { 25, 19, 13, 10, 7, 5, 4 }; // Add a value for LOW_6

    for (uint8_t p = 0; p < 3; p++)
    {
        if (currentPower < OUTPUT_POWER_MID)
        {
            Txp[p] = (Txp[p] * 4) / dividers[currentPower];
        }
        else if (currentPower == OUTPUT_POWER_MID)
        {
            Txp[p] = (Txp[p] * 3) / dividers[currentPower];
        }
        else if (currentPower == OUTPUT_POWER_HIGH)
        {
            Txp[p] += 30; // or your custom logic for 8W
        }
    }
#else
